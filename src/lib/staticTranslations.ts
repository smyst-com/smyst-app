...
export interface StaticTranslations {
  ...
  upload: { storagePaused: string; emptyFile: string; tooLarge: string; wrongType: string; aborted: string }
  langSw: { ariaCurrent: string; searchLabel: string; searchPlaceholder: string }
  voiceWave: { speaking: string; listenDictation: string; listenWave: string; paused: string; preparing: string; detailSpeaking: string; detailDictation: string; detailWave: string; detailPaused: string; detailPreparing: string }
  menuA11y: { open: string; close: string; startMenu: string; startMenuNav: string }
  chatA11y: { openProfile: string; addPhoto: string; openCamera: string; addAudio: string }
  privacyStatus: { profileSaved: string; memorySaved: string; memoryDeleted: string; pkAccepted: string; pkRejected: string; exportCreated: string; accountDeleted: string }
  greet: { withName: string; withoutName: string }
  chatBar: { noProfile: string; selectProfile: string; addMedia: string; messageTo: string; selectPublicProfile: string; selectAiProfileFirst: string; voiceInput: string; voiceInputUnsupported: string; sendMessage: string; notePlaceholder: string; messageMenu: string; copy: string; speak: string; speakStop: string; forward: string }
...
export const DEFAULT_TRANSLATIONS: StaticTranslations = {
  ...
  upload: { storagePaused: 'Upload-Speichern ist gerade wegen eines temporären Speicherlimits pausiert. Bitte versuche es später erneut.', emptyFile: 'Datei ist leer.', tooLarge: 'Datei ist fuer diese Kategorie zu gross.', wrongType: 'Dateityp passt nicht zur gewählten Kategorie.', aborted: 'Upload abgebrochen.' },
  langSw: { ariaCurrent: 'Sprache wechseln, aktuell:', searchLabel: 'Sprache suchen', searchPlaceholder: 'Suche … (z. B. español, türkçe, japan)' },
  voiceWave: { speaking: 'Twin spricht laut', listenDictation: 'Diktat hört zu', listenWave: 'Sprachwelle hört zu', paused: 'Sprachwelle pausiert', preparing: 'Antwort wird vorbereitet', detailSpeaking: 'Laut vorlesen aktiv', detailDictation: 'Sprich deinen Text – er erscheint im Eingabefeld. Zum Stoppen Mikrofon erneut tippen.', detailWave: 'Sprich weiter. Nach kurzer Ruhe wird automatisch gesendet.', detailPaused: 'Tippe die Welle erneut, um weiterzusprechen.', detailPreparing: 'Der Twin antwortet gleich und liest danach laut vor.' },
  menuA11y: { open: 'Menü öffnen', close: 'Menü schließen', startMenu: 'Startmenü', startMenuNav: 'Startmenü Navigation' },
  chatA11y: { openProfile: 'Profil von {name} öffnen', addPhoto: 'Foto oder Video hinzufügen', openCamera: 'Kamera öffnen', addAudio: 'Audio hinzufügen' },
  privacyStatus: { profileSaved: 'Profil gespeichert und für IDrive-e2-Objektpersistenz vorgemerkt.', memorySaved: 'Memory gespeichert.', memoryDeleted: 'Memory gelöscht.', pkAccepted: 'Public Knowledge nach Review übernommen.', pkRejected: 'Public-Knowledge-Vorschlag abgelehnt.', exportCreated: 'Export erstellt.', accountDeleted: 'Account-Löschung ausgeführt. Du wirst abgemeldet.' },
  greet: { withName: 'Hallo! Hier ist {name}. Schön, dass du da bist. Was kann ich für dich tun?', withoutName: 'Hallo! Schön, dass du da bist. Was kann ich für dich tun?' },
  chatBar: { noProfile: 'Kein Profil ausgewählt', selectProfile: 'Profil auswählen', addMedia: 'Medien hinzufügen', messageTo: 'Nachricht an {name}...', selectPublicProfile: 'Öffentliches Profil auswählen', selectAiProfileFirst: 'Zuerst KI-Profil auswählen', voiceInput: 'Spracheingabe', voiceInputUnsupported: 'Spracheingabe nicht unterstützt', sendMessage: 'Nachricht senden', notePlaceholder: 'Kernidee, einfache Erklärung, Folgefragen …', messageMenu: 'Nachrichtenmenü öffnen', copy: 'Kopieren', speak: 'Vorlesen', speakStop: 'Vorlesen stoppen', forward: 'Weiterleiten' },
...
function mergeTranslations(value: Partial<StaticTranslations>): StaticTranslations {
  return {
    ...
    upload: { ...DEFAULT_TRANSLATIONS.upload, ...value.upload },
    langSw: { ...DEFAULT_TRANSLATIONS.langSw, ...value.langSw },
    voiceWave: { ...DEFAULT_TRANSLATIONS.voiceWave, ...value.voiceWave },
    menuA11y: { ...DEFAULT_TRANSLATIONS.menuA11y, ...value.menuA11y },
    chatA11y: { ...DEFAULT_TRANSLATIONS.chatA11y, ...value.chatA11y },
    privacyStatus: { ...DEFAULT_TRANSLATIONS.privacyStatus, ...value.privacyStatus },
    greet: { ...DEFAULT_TRANSLATIONS.greet, ...value.greet },
    chatBar: { ...DEFAULT_TRANSLATIONS.chatBar, ...value.chatBar },

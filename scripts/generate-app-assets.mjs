...
// App-Store-Icons duerfen keinen Alpha-Kanal haben; das Brand-Symbol in
// logo.svg ist transparent, daher wird die dunkle Markenflaeche unterlegt.
const BRAND_BACKGROUND = '#111722'

async function writePng(file, size, { flatten = true } = {}) {
  await fs.mkdir(path.dirname(file), { recursive: true })
  let image = sharp(logoSvg).resize(size, size)
  if (flatten) {
    image = image.flatten({ background: BRAND_BACKGROUND })
  }
...
for (const [density, size] of Object.entries(androidSizes)) {
  ...
  await writePng(path.join(dir, 'ic_launcher_foreground.png'), size, { flatten: false })
}
...
<svg width="2732" height="2732" viewBox="0 0 2732 2732" xmlns="http://www.w3.org/2000/svg">
  ...
  <g transform="translate(1166 1166) scale(0.390625)">
    ${innerLogo}
  </g>
</svg>
`)
